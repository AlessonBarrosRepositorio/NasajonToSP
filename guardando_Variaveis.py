...
mes_digitado = input("Digite o nome do mês (ex: Janeiro): ")

numero_mes = MesesAno.get(mes_digitado)

if numero_mes:
    print(f"O número do mês {mes_digitado} é {numero_mes}.")
else:
...
Relação de Salários Líquidos em Março de 2025Página 5 de 80
(Adiant. de Salário)
Conta Salário
Funcionário CPF Banco Agência Valor Conta
...
for bloco in blocos:
    match_data = re.search(r'Relação de Salários Líquidos em (.+?)(?:Página|\n)', bloco)
    if match_data:
        data = match_data.group(1).strip()
        funcionarios.append({"dataReferencia": data})

# Imprime a data de referência de cada funcionário
