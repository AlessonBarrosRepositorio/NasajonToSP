...
class AplicativoProcessadorArquivos:
    def __init__(self, root):
        ...
        self.dia_pagamento = tk.StringVar(value="05")  # Valor padrão
    ...
    def criar_interface(self):
        ...
        # Dia de pagamento
        tk.Label(frame_principal, text="Dia de pagamento:").grid(row=2, column=0, sticky="w", pady=(0, 5))
        tk.Entry(frame_principal, textvariable=self.dia_pagamento, width=5).grid(row=3, column=0, sticky="w", pady=(0, 10))
    ...
    def processar_arquivo(self):
        ...
        dia_pagamento = self.dia_pagamento.get().zfill(2)
        
        if not arquivo_pdf:
            messagebox.showerror("Erro", "Por favor, selecione um arquivo PDF.")
            return
        
        if not dia_pagamento.isdigit() or len(dia_pagamento) != 2:
            messagebox.showerror("Erro", "Por favor, insira um dia de pagamento válido (2 dígitos).")
            return
        ...
        try:
            ...
            for bloco in blocos_persona:
                funcionarios.extend(self.extrair_dados_funcionarios(bloco, dia_pagamento))
    ...
    def extrair_dados_funcionarios(self, bloco, dia_pagamento):
        ...
        if match_data:
            ...
            numero_mes = self.meses_ano.get(nome_mes, '00').zfill(2)
            data_formatada = f"{numero_mes}{ano}"
    ...
    def salvar_arquivo(self):
        ...
        try:
            caminho_final = os.path.join(pasta_destino, "resultado_final.txt")
